...
- **API error responses are normalized and actionable.** `services/api/src/common/http-exception.filter.ts` turns every thrown error into a stable envelope containing a safe message, HTTP status, machine-readable code, retry guidance, and an error reference that matches the server log. Validation failures include every invalid field, while database/network failures are classified without exposing SQL, stack traces, credentials, or constraint names. See [`docs/api-error-responses.md`](docs/api-error-responses.md).
...
- Each site can display that opt-in UI immediately, after a configurable scroll depth (1–100%), or on a configurable page view (1–100). Page-view progress is stored per site in the visitor's browser; existing sites default to immediate display. The SDK refreshes the public site configuration through a versioned five-minute URL before applying this rule, so stale platform/CDN page markup cannot pin an older display mode while shared edge caching remains effective.
